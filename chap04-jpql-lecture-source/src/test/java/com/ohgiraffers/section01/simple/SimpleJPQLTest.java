...
public class SimpleJPQLTest {
    ...
    public void TypedQuery를_이용한_단일행_단일열_조회_테스트() {
        ...
        String resultMenuName = query.getSingleResult();            // 결과가 단일행임을 알고 getStringReuslt() 호출
...
//        Menu testMenu = entityManager.find(Menu.class, 1);
//        System.out.println("testMenu = " + testMenu);
    ...
    public void distinct를_활용한_중복제거_여러_행_조회_테스트() {
        ...
        List<Integer> foundCategoryCodeList = query.getResultList();
        foundCategoryCodeList.forEach(System.out::println);

        Assertions.assertTrue(!foundCategoryCodeList.isEmpty());
    }
    ...
    /* 설명. jpql은 기본적으로 SQL문과 크게 다르지 않지만 set operator나 아니면 subquery 활용 등에서차이가 나는 부분이 있다. */
}
